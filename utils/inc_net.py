...
class EMAIncrementalNet(BaseNet):
    def __init__(self, args, pretrained, gradcam=False):
        super().__init__(args, pretrained)
        self.gradcam = gradcam
        if hasattr(self, "gradcam") and self.gradcam:
...
class AKAIncrementalNet(BaseNet):
    def __init__(self, args, pretrained, gradcam=False):
        super().__init__(args, pretrained)
        self.gradcam = gradcam
        self.fc = None
    ...
    def update_fc(self, nb_classes, sla_n):
        aug_fc = self.generate_fc(self.feature_dim, nb_classes)
        fc = self.generate_fc(self.feature_dim, nb_classes//sla_n)
        if self.aug_fc is not None:
            nb_output = self.aug_fc.out_features
            weight = copy.deepcopy(self.aug_fc.weight.data)
            bias = copy.deepcopy(self.aug_fc.bias.data)
            aug_fc.weight.data[:nb_output] = weight
            aug_fc.bias.data[:nb_output] = bias
